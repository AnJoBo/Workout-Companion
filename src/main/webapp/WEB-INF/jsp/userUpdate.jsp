		...
		<div class="col-md-5">
			...
			<form action="${formAction}" method="POST">
				...
				<div class="form-group">
					...
					<img src="${userImg}" style="display: block;"> <input
						type="file" id="picture" name="newPicture" class="form-control" />
				</div>

				<div class="form-group">
					<label for="newUserName">Username: </label> <input type="text"
						id="newUserName" name="newUserName" class="form-control"
						placeholder="${currentUser.userName}" />
				</div>

				<div class="form-group">
					<label for="userName">Email: </label> <input type="email"
						id="email" name="newEmail" class="form-control"
						placeholder="${currentUser.email}" />
				</div>

				<div class="form-group">
					<label for="userName">Phone Number: </label> <input type="text"
						id="phone" name="newPhone" class="form-control"
						placeholder="${currentUser.phone}" />
				</div>

				<div class="form-group">
					<label for="fitnessGoal">Select Fitness Goal:</label> <select
						class="form-control" name="newFitnessGoal">
						<option value="maintain weight"
							${currentUser.fitnessGoal == "maintain weight" ? "selected" : "" }>MAINTAIN
							WEIGHT</option>
						<option value="lose weight"
							${currentUser.fitnessGoal == "lose weight" ? "selected" : "" }>LOSE
							WEIGHT</option>
						<option value="gain weight"
							${currentUser.fitnessGoal == "gain weight" ? "selected" : "" }>GAIN
							WEIGHT</option>
					</select>
				</div>

				<div class="form-group">
					<label for="newPassword" style="color: red;">Enter
			...
			<form action="${formAction}" method="POST" id="changePasswordForm">
				...
				<div class="form-group">
					<label for="newPassword" style="color: red;">Current
						Password: </label> <input type="password" id="password" name="password"
						class="form-control" />
				</div>
